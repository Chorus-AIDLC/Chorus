...
| Tool | Purpose |
|------|---------|
| `chorus_create_session` | Create a session for a sub-agent (manual on dsh — see below) |
...
### Step 8.5: Run the Task Reviewer (inline — no hook on dsh)

> **dsh difference:** the Claude Code plugin relies on a PostToolUse hook to inject a "spawn the reviewer" reminder after `chorus_submit_for_verify`. **dsh has no such hook.** Run the reviewer step **inline**, right here, immediately after submitting. Do not wait for an injected reminder.
...
If you spawned a sub-agent and no new `VERDICT:` comment appears after it returns, it exhausted its turn budget. **Not every missing VERDICT is silence:** if the reviewer instead posted a `REVIEW SKIPPED:` comment (round limit reached — human decision needed) or any other explicit refusal to review, that is a deliberate escalation — STOP: do not respawn, do not self-review, and do not post a VERDICT of your own; leave the entity as it is for the human to decide. If it truly posted nothing, respawn it ONCE with a concise-budget hint: *"Stay within turn budget. Skip deep verification. Fetch task/proposal/comments, run only the core tests, and post your VERDICT within the first 12 turns."* If the second attempt still produces no VERDICT, fall back to reviewing manually (Step 8.5 fallback) and post the VERDICT yourself.
...
If the task you just self-verified was the LAST one of its Idea and you have `document:write`, create the completion report when the wake explicitly requests it. Otherwise leave it for the orchestrator; in a daemon-headless turn, do not block task completion on an interactive report choice.

---

## Session (Sub-Agents Only)

> **dsh difference:** session lifecycle is **manual** on dsh. The Claude Code plugin automates creation, heartbeat, and cleanup via hooks; dsh does not run those hooks. A sub-agent therefore manages its own session:
...
Session handling is MANUAL on dsh:
...
If you dispatch sub-agents, ensure they can reach the Chorus MCP server — the plugin config (and therefore the `mcp__chorus__*` tools) must be available in the sub-agent's environment, and the API key must carry the needed permissions.
...
| Duplicate sessions | On dsh the sub-agent creates its own session — if it created several, close extras via `chorus_close_session` or the Settings page |
...
- Always run the reviewer inline after submit (Step 8.5) — dsh has no hook to remind you
- Sessions are manual on dsh — create, checkin/checkout, and close your own session as a sub-agent
...
- After submitting for verification, an Admin reviews using `review-chorus`
