...
In Kiro, hand the reviewer this task (auto-selected by its description; also invokable as /chorus-task-reviewer): \"Review task ${TASK_UUID}. Max review rounds: ${MAX_ROUNDS}. First, read existing comments to count previous VERDICTs and determine your round number. If max > 0 and your round exceeds max, skip the review and post a comment whose first line is exactly REVIEW SKIPPED: round limit reached — human decision needed (this marker tells the main agent to stop rather than substitute its own verdict). Otherwise, proceed with the review and post your VERDICT as a comment on the task.\"
...
IMPORTANT: the \`subagent\` call's own return value is not the verdict — base the decision on the comment. Do NOT verify or reopen before you have read this round's VERDICT comment. If no such comment exists after the reviewer completes, check what the reviewer actually did post before retrying — not every missing VERDICT is silence: if the reviewer instead posted a REVIEW SKIPPED: comment (round limit reached, human decision needed) or any other explicit refusal to review, that is a deliberate escalation — STOP: do not respawn, do not self-review, and do not post a VERDICT of your own; leave it pending the human's decision. If it truly posted nothing, respawn it once with a concise-budget hint; if still absent, review the task yourself as a read-only pass and post the VERDICT. Absence is never a PASS."
