...
- ⚠️ **OPEN — not yet closed:**
  ...
  2. **No dsh-agent comment posted** on the fixture (0 comments) despite the prompt asking for one — the agent's Chorus **MCP-action round-trip is unconfirmed** (transcript upload is done by the daemon, not the agent's MCP). Could be MCP-cred wiring in the composed bundle, or agent behavior.
...
- [~] **AC1 turn (core proven, edges open)** — compose+spawn+turn+idea-anchored transcript proven live (run 2). Open: `source=dsh` usage frame + agent MCP-action round-trip + dsh-session-id transcript 404 (see above).
