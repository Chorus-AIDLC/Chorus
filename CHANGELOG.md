...
- **Migration impact** — rotating the JWT secret invalidates every existing session, so read the row that matches your deployment:
  ...
  - (d) **Explicit secure `NEXTAUTH_SECRET`, AWS CDK deployment, or the npm `chorus` launcher** — no change. CDK already injects a random value from Secrets Manager and the launcher already persists its own generated secret to `~/.chorus/.secret`.
