...
2. **Create the empty proposal container.** In OpenSpec mode the `description` MUST contain `OpenSpec change slug: <slug>`; in spec-lite mode the locator line `Spec-lite: .chorus/specs/<slug>/ (change: changes/YYYY-MM-DD-<change-slug>.md)` (use the `$SLUG` you'll pick in 2a / 2c); in free-form mode omit any slug line.

   ```
   chorus_pm_create_proposal({
     projectUuid: "<project-uuid>",
     title: "<feature name>",
     description: "<summary>\n\nSpec-lite: .chorus/specs/<slug>/ (change: changes/YYYY-MM-DD-<change-slug>.md)",  // spec-lite mode
   ...
   **2c. spec-lite mode (resolved mode = lite).** Load the `spec-lite` skill (`skills/spec-lite/SKILL.md`) and follow it: pick `$SLUG` (a **capability/feature** name — the durable spec folder `.chorus/specs/<slug>/` is edited in place across changes, not one folder per change). Ensure `prd.md` exists, copying from `.chorus/specs/TEMPLATE/prd.md` the first time this capability is specced (or the skill's inline shape if the template isn't present) — Chorus PRD: Intent / plain-prose Requirements + `- [ ]` acceptance points / Non-goals, plus `tech_design.md` etc. only if warranted. Write this effort's dated change doc `changes/YYYY-MM-DD-<change-slug>.md` (What / Why / Acceptance, from `.chorus/specs/TEMPLATE/changes/YYYY-MM-DD-change.md`), then edit the durable docs in place to the new truth. The proposal `description` MUST carry the literal locator line `Spec-lite: .chorus/specs/<slug>/ (change: changes/YYYY-MM-DD-<change-slug>.md)` (the deterministic link develop uses). Mirror **each** touched durable `<type>.md` to its persistent Document byte-exact — first time a doc is specced e.g. `chorus mcp call chorus_pm_add_document_draft "{\"proposalUuid\":\"<uuid>\",\"type\":\"prd\",\"title\":\"PRD: <feature>\"}" --arg-file content=.chorus/specs/<slug>/prd.md`, later edits via `chorus_pm_update_document` against the recorded `documentUuid`. No `OpenSpec change slug:` line and no `openspec/changes/` scaffold; there is no `tasks.md`. Then continue to step 3 for task drafts.
