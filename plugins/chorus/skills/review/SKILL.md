...
3. **No new VERDICT comment?** Check what the reviewer *did* post. A `REVIEW SKIPPED:` comment or any other explicit refusal to review is a deliberate escalation to a human: STOP — do not respawn, do not self-review, do not post a VERDICT of your own. If it posted nothing at all, respawn it ONCE, telling it to stay within its turn budget and reserve its last turns for the VERDICT. If it is still silent, review the item yourself as a read-only pass using the checklists below and POST the VERDICT — **absence is never a PASS**.
4. **Track rounds.** Count existing VERDICT comments before spawning. After 3 rounds of FAIL on the same item, stop the loop and escalate to human review: post a comment whose first line is exactly `REVIEW SKIPPED: round limit reached — human decision needed`, and post no VERDICT. Nobody — including you on a later turn — may replace that marker with a self-reviewed VERDICT.
