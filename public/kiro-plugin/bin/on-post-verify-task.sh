...
2. Read THIS round's VERDICT on the idea (chorus_get_comments targetType=\"idea\") — the comment posted after your dispatch, not an earlier round's. The \`subagent\` call's own return value is not the verdict; do not declare the feature shippable before you have read that comment. If it is missing, check what the reviewer did post: a REVIEW SKIPPED: comment (round limit reached, human decision needed) or any other explicit refusal is a deliberate escalation — STOP: do not respawn, do not self-review, do not post a VERDICT of your own; leave it pending the human's decision. If it posted nothing at all, respawn once; if still absent, review the idea's aggregate change yourself as a read-only pass and post the VERDICT. Absence is never a PASS.
