...
> **Document update flow (spec-lite mode):** if the originating proposal `description` contains a line `Spec-lite change slug: <slug>`, the project's `prd` / `tech_design` / … Documents are **mirrors** of the files in `.chorus/specs/<slug>/` (`prd.md`, `tech_design.md`, …, named by Chorus Document type). Load the `spec-lite` skill (`skills/spec-lite/SKILL.md`). Locate the folder **deterministically** by that slug line (not by title/type, which isn't unique). Edit the `<type>.md` file first (it is the source of truth), tick `- [ ]` acceptance points as work completes, then re-mirror each edited file via `chorus mcp call chorus_pm_update_document … --arg-file content=.chorus/specs/<slug>/<type>.md`. No new tool/CLI — same `--arg-file` transport as OpenSpec. **Git history is the audit trail** (`git log -- .chorus/specs/<slug>/` for the whole change; `git log --follow -- <file>` to trace a single renamed file) — there is no changelog section to maintain.
>
