...
IMPORTANT: the \`subagent\` call's own return value is not the verdict — base the decision on the comment. Do NOT verify or reopen before you have read this round's VERDICT comment. If it is missing, check what the reviewer did post: a REVIEW SKIPPED: comment (round limit reached, human decision needed) or any other explicit refusal is a deliberate escalation — STOP: do not respawn, do not self-review, do not post a VERDICT of your own; leave it pending the human's decision. If it posted nothing at all, respawn once; if still absent, review the task yourself as a read-only pass and post the VERDICT. Absence is never a PASS."
