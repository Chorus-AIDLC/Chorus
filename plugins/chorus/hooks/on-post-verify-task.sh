...
ACTION REQUESTED: spawn code-reviewer for this idea — an independent, read-only review of the idea's AGGREGATE code change (the whole feature across all its tasks), not a single task. Mount the chorus-code-reviewer skill explicitly with spawn_agent({items:[{type:"skill", path:"chorus:chorus-code-reviewer"}, {type:"text", text:"Review aggregate code for idea ${idea_uuid}. Round: <N>. Post VERDICT."}]}), where the round number comes from prior code-review verdict comments on the idea. It reviews cross-task integration, architecture/convention consistency, security, regression/performance, feature-level test coverage, and overall code soundness, then posts ONE VERDICT comment on the idea. Read its VERDICT (chorus_get_comments targetType="idea"): PASS / PASS WITH NOTES -> may ship; FAIL -> the orchestrator must invoke Quick Dev to create Proposal-linked fix tasks (group related small BLOCKERs; split only materially large or independently testable work; never reopen completed tasks or apply untracked fixes). Re-run only after every fix task passes AC self-check, independent task review, and admin verification to `done`; failed or cancelled fixes stop and escalate. Keep the maxCodeReviewRounds cap. Run the code-review gateway BEFORE writing any idea-completion report.
CTX
}
