...
// Project Members API - List, Add, Remove (Project Visibility — Tech Design §5)
// UUID-Based Architecture: All operations use UUIDs
//
// Leak rule: an inaccessible project must look like it does not exist (404).
// An accessible project the actor cannot MANAGE (i.e. is not the owner) yields
...
// GET /api/projects/[uuid]/members - List project members
export const GET = withErrorHandler<{ uuid: string }>(
  async (request: NextRequest, context: RouteContext) => {
    ...
    // Must be able to access the project; otherwise hide its existence.
    if (!(await canAccessProject(auth, projectUuid))) {
      return errors.notFound("Project");
    }

    const members = await listProjectMembers(auth.companyUuid, projectUuid);
    return success({ members });
  }
);

// POST /api/projects/[uuid]/members - Add a member (owner-only)
...
// DELETE /api/projects/[uuid]/members?memberType=...&memberUuid=... - Remove a member (owner-only)
