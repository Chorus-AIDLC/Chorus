...
const IDEA_BADGE_HINT: Record<string, IdeaHintEntry> = {
  open: { labelKeySuffix: "open", fg: "#888780" },
  researching: { labelKeySuffix: "researching", fg: "#7F77DD" },
  answer_questions: { labelKeySuffix: "answerQuestions", fg: "#C47A20" },
  planning: { labelKeySuffix: "planning", fg: "#7F77DD" },
  review_proposal: { labelKeySuffix: "reviewProposal", fg: "#C47A20" },
  building: { labelKeySuffix: "building", fg: "#7F77DD" },
  verify_work: { labelKeySuffix: "verifyWork", fg: "#C47A20" },
  done: { labelKeySuffix: "done", fg: "#1D9E75" },
  closed: { labelKeySuffix: "closed", fg: "#888780" },
};
...
export const UNKNOWN_FALLBACK: NodeStatusVisual = {
  ...
  colorClass: "bg-[#F5F5F5] text-[#6B6B6B]",
  bg: "#F5F5F5",
  fg: "#6B6B6B",
};
...
/** Parse a `bg-[#RRGGBB] text-[#RRGGBB]` Tailwind pair into hex strings. */
function splitColorClass(colorClass: string): { bg: string; fg: string } {
  const bgMatch = colorClass.match(/bg-\[(#[0-9A-Fa-f]{3,8})\]/);
  const fgMatch = colorClass.match(/text-\[(#[0-9A-Fa-f]{3,8})\]/);
  return {
    bg: bgMatch ? bgMatch[1] : UNKNOWN_FALLBACK.bg,
    fg: fgMatch ? fgMatch[1] : UNKNOWN_FALLBACK.fg,
  };
}
...
export function resolveNodeStatusVisual(
  ...
  if (type === "idea") {
    ...
    const colorClass = `bg-[${IDEA_BADGE_HINT_CHIP_BG}] text-[${entry.fg}]`;
    return {
  ...
  if (type === "document") {
    ...
    const { bg, fg } = splitColorClass(colorClass);
    return { labelKey, colorClass, bg, fg };
  }
  ...
  const { bg, fg } = splitColorClass(colorClass);
  return { labelKey: `status.${suffix}`, colorClass, bg, fg };
}
