...
A dismissed attempt that succeeds while still current enters `confirmed`, preserving drafts and a submission guard across close/reopen. A translated banner states that the prior request created a project. The user must explicitly confirm creating another project before the ordinary submit path can run. A superseded attempt still only refreshes data. Directly clearing drafts in the shared late-success branch is unsafe: it also handles superseded attempts, and even the same dismissed attempt may have been reopened and edited.

Only `attempt.controller.signal.aborted` suppresses a validation error. A rejection merely named AbortError from another source is visible and releases validation for deliberate retry; an AbortError after POST remains an unconfirmed result.

The cross-group warning context and visibility of snapshot submission semantics are recorded in follow-up Idea d4a00d67-3399-445a-9252-295811910317. Pencil's explicit user waiver continues to apply.
