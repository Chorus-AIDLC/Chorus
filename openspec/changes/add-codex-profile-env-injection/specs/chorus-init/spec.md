...
`[shell_environment_policy].set` governs the environment of Codex's exec/shell tool, so this write covers the model's own `chorus` shell calls. Whether it also reaches Codex's plugin lifecycle hook subprocesses (a separate spawn path) is verified by a spike. If the hooks are found NOT to inherit `[shell_environment_policy].set`, `chorus agents add` MUST surface the residual hook gap via an actionable message plus the manual `export CHORUS_URL / CHORUS_API_KEY / CHORUS_AGENT_PROFILE` hint — it MUST NOT introduce a wrapper and MUST NOT silently ignore the gap.

On a successful write the command SHALL suppress the manual `export CHORUS_AGENT_PROFILE` hint for that agent (the config now carries the creds), mirroring the dsh `$DSH_HOME/.env` and Claude Code `~/.claude/settings.json` behavior.
