...
A **shared sub-procedure** for the Chorus stage skills (`proposal-chorus`, `develop-chorus`, `yolo-chorus`) — the
lightweight spec mode, modelled on **superpowers** (a durable spec that lives on, plus per-effort artifacts):
...
The spec mode is computed by the **chorus-dsh bundle at plugin load** (`resolveSpecMode`, the TS mirror of
the canonical bash resolver), **not by you** — the bundle publishes it as the `CHORUS_SPEC_MODE`
environment variable and injects a `## Spec Mode` section into your first-step context stating the resolved
value. You are here because it resolved to `lite`; if `## Spec Mode` (or `CHORUS_SPEC_MODE`) says anything
else, this skill is a no-op — return to the caller. (For the record, the rule: an explicit `CHORUS_SPEC_MODE`
wins, else OpenSpec when usable, else lite.)
...
**halt**). Guard every call with the `chorus_check_response` halt-on-error helper (`openspec-aware-chorus`
§6). No `chorus` on `PATH`? Fall back to the package-local `chorus-mcp-call.mjs` wrapper (resolved into
`$CHORUS_MCP_CALL` at plugin load) + `json_encode_file` (`openspec-aware-chorus` §3.6). **`<slug>/spec.md`
is NEVER in this loop.**
...
Fallback shape (no `chorus` on `PATH`) — build `$PAYLOAD` with `json_encode_file` and call
`"$CHORUS_MCP_CALL" chorus_pm_update_document "$PAYLOAD"`, then `chorus_check_response` (see
`openspec-aware-chorus` §3.6–§3.8 for the exact fallback blocks).
